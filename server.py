...
# -------------------------------------------------------------
@app.get("/api/market/lhb", summary="拉取公开龙虎榜并构建主力资金天团引力抱团星图")
def get_market_lhb(date: str = None):
    # 1. 确定基准日期
    con = duckdb.connect()
    date_str = date
    if not date_str:
        try:
            latest_date_df = con.execute(f"SELECT MAX(date) AS mdate FROM read_parquet('{DATA_STORE_DIR}/sh600000.parquet')").fetchdf()
            latest_date = latest_date_df.iloc[0]['mdate']
    ...
    # 3. 异步回溯拉取东财 LHB 接口 (RPT_BILLBOARD_DAILYDETAILSBUY)
    import requests
    lhb_data = None
